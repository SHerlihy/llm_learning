...
.setTag('prompting')
.setTag('agent')
.setFlashcard('Shadow AI', 'Unsanctioned AI tools and applications deployed within an organization without IT vetting or oversight, bypassing standard security reviews.')
.setFlashcard('AI agents', 'Autonomous entities designed to perform specific tasks and navigate systems with minimal human oversight, acting as the building blocks of agentic AI.')
.setFlashcard('Adaptive malware', 'Malware that integrates LLM APIs to generate malicious code or commands on demand during execution, allowing it to be polymorphic and harder to detect.')
.setFlashcard('Vibe coding', 'The use of generative AI as a productivity multiplier for tasks like troubleshooting code, researching vulnerabilities, and drafting content.')
.setFlashcard('AI Red Teaming', 'An offensive security practice involving the active stress-testing of AI systems to identify vulnerabilities and validate defensive controls.')
.setFlashcard('Indicators of Activity (IOAs)', 'Telemetry focusing on behavioral anomalies and illogical sequences of actions, used to detect dynamic threats that static signatures might miss.')
.setFlashcard('Prompt injection', 'An attack vector that subverts an AI\'s intended function by exploiting its ability to process and act on hidden or malicious prompts.')
.setFlashcard('Excessive Agency', 'A design flaw where an AI agent is permitted to access information or perform tasks that exceed its authorized business requirements.')
.setFlashcard('Model Context Protocol (MCP)', 'A standard for granting agentic applications access to data and enabling their integration into enterprise tools.')
.build()
